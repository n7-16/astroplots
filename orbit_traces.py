...
sizescale = 150 # pixels = 1 simulated AU
timescale = 10 # seconds = 1 simulated year 

class planet(pygame.sprite.Sprite):
    def __init__(self, a, e, omega, _display_surf, m, name,color = "red",custom=False):
        super().__init__()
    ...
    def on_init(self):
        ...
        try:
            ...
            print(self.semimajor*2+10,self.semiminor*2+10)
    ...
    def draw_elip(self, _render_surf, _target_rect, color="red"):
        ...
        if not self.custom:
            ...
            pygame.draw.ellipse(self._render_surf, (255, 255, 0, 80),(0, 0, self._target_rect.width, self._target_rect.height), 4)
...
class run():
    def __init__(self):
        global height, width, sizescale, timescale
        with open("parameters.txt", "r") as f:
            sizescale = np.float64(f.readline().strip())
            timescale = np.float64(f.readline().strip())
        # print(sizescale,timescale)
        ...
        self.uranus = planet(2873.46/150*sizescale, 0.0472, 0, self._display_surf, 8.681*(10**25), name="uranus", color=(79, 158, 215))
        self.neptune = planet(4495.06/150*sizescale, 0.0086, 0, self._display_surf, 1.02413*(10**26), name="neptune", color=(33, 64, 243))
        ...
        # self.planetlst = [self.sun, self.earth, self.mercury, self.venus, self.mars, self.jupiter, self.saturn, self.uranus, self.neptune]
        with open("planetsworking.csv", "r") as f:
            reader = csv.DictReader(f)
            for row in reader:
                # print(row)
                planet_obj = planet(
                    np.float64(row["a (AU)"])*sizescale, np.float64(row["e"]), 0, self._display_surf, 
                    np.float64(row["m (kg)"]), "custom planet", (255,0,0), custom=True
                )
                self.planetlst.append(planet_obj)
